...
class Home(webapp.RequestHandler):
    def get(self):
		...
		feed = client.GetDataFeed(data_query)                                       	
        	...
        	counter = feed.entry[0].metric[0].value
        
        template_values={'allAppsList': allAppsList, 'userStatus': userStatus, 'counter': counter}
...
class ProfileHandler(webapp.RequestHandler):
    def get(self):
        ...
        if account:  # dude has already registered
            ...
            ifEducator = ''
            educationLevel = ''
            displayName=str(user)                #default displayname is email
            user = users.get_current_user()
            account = Account()
            account.user  = user
            account.firstName = ''
        ...
        template_values={'account': account, 'currenttime':currenttime, 'allAppsList': allAppsList, 'allAppsList2': allAppsList2, 'userStatus': userStatus,
...
class CourseOutlineHandler(webapp.RequestHandler):
    def get(self):
        ...
        template_values={'currenttime':currenttime, 'allAppsList': allAppsList, 'allAppsList2': allAppsList2, 'userStatus': userStatus}
...
class TryItHandler(webapp.RequestHandler):
    def get(self):
        ...
        template_values={'currenttime':currenttime, 'allAppsList': allAppsList, 'allAppsList2': allAppsList2, 'manyMoldAppsList': manyMoldAppsList, 'userStatus': userStatus}
...
class IHaveADreamHandler(webapp.RequestHandler):
    def get(self):
        ...
        allAppsList = cacheHandler.GettingCache("App", True, "version", "1", True, "number", "ASC", True)
        allAppsList2 = cacheHandler.GettingCache("App", True, "version", "2", True, "number", "ASC", True)
        template_values={'currenttime':currenttime, 'allAppsList': allAppsList, 'allAppsList2': allAppsList2, 'apps2Dir':APPS2DIR}
...
class QuizlyHandler(webapp.RequestHandler):
	...
	quizName= self.request.get('quizname')
        ...
        allAppsList = cacheHandler.GettingCache("App", True, "version", "1", True, "number", "ASC", True)
        allAppsList2 = cacheHandler.GettingCache("App", True, "version", "2", True, "number", "ASC", True)
        template_values={'currenttime':currenttime, 'allAppsList': allAppsList, 'allAppsList2': allAppsList2, 'apps2Dir':APPS2DIR,'quizname':quizName}
...
class NewAppRenderer_AI2(webapp.RequestHandler):
    def get(self):
        ...
        path = os.path.join(os.path.dirname(__file__),'static_pages/other/app_base_new_ai2.html')
        self.response.out.write(template.render(path, template_values))
...
class TryItHandler(webapp.RequestHandler):
    def get(self):
        ...
        template_values={'currenttime':currenttime, 'allAppsList': allAppsList, 'allAppsList2': allAppsList2, 'userStatus': userStatus, 'manyMoldAppsList': manyMoldAppsList}
        path = os.path.join(os.path.dirname(__file__),'static_pages/other/tryit.html')
        self.response.out.write(template.render(path, template_values))
...
class TeacherMapHandler(webapp.RequestHandler):
    def get(self):
        ...
        currenttime = datetime.utcnow()
        template_values={'currenttime':currenttime, 'allAppsList': allAppsList, 'allAppsList2': allAppsList2, 'accounts': accounts, 'account_k_8':account_k_8,  'account_high_school':account_high_school, 'account_college_university':account_college_university, 'userStatus': userStatus}
        ...
        for account in accounts:
            ...
            try:
                ...
                if firstName == None or lastName == None:
                    account.displayName = str(account.user)
                    account.put()
